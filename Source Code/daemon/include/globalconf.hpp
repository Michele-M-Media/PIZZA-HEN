...
#define ORIGINAL_ELF_PORT 9020 // Replace with the actual port number
#include <inttypes.h>
...
extern "C" int sceNetSocket(const char *name, int domain, int type,
                            int protocol);
...
enum {
  ...
  SCE_VIDEO_OUT_BUFFER_INDEX_BLANK =
      -1, /* special buffer index to transparent blank screen */
  SCE_VIDEO_OUT_BUFFER_INDEX_BLACK =
      -2, /* special buffer index to opaque black screen */
  SCE_VIDEO_OUT_BUFFER_INITIAL_FLIP_ARG =
      -1, /* initial flipArg valu at sceVideoOutOpen() */
};
...
typedef enum SceVideoOutFlipMode {
  ...
  SCE_VIDEO_OUT_FLIP_MODE_WINDOW =
      3, /* similar to vsync but may flip on some windows at the top and the
            bottom of the display. */
  SCE_VIDEO_OUT_FLIP_MODE_VSYNC_MULTI =
      4, /* vsync mode but allows multiple flips per vsync. flipRate is not
            valid. */
...
static const uint64_t SCE_VIDEO_OUT_BUFFER_ATTRIBUTE_OPTION_STRICT_COLORIMETRY =
    (1UL << 3);
static const uint64_t
    SCE_VIDEO_OUT_BUFFER_ATTRIBUTE_OPTION_ALPHA_NON_PREMULTIPLIED = (0UL << 5);
static const uint64_t
    SCE_VIDEO_OUT_BUFFER_ATTRIBUTE_OPTION_ALPHA_PREMULTIPLIED = (1UL << 5);
static const uint64_t
    SCE_VIDEO_OUT_BUFFER_ATTRIBUTE_OPTION_ALPHA_PREMULTIPLIED_MASK = (1UL << 5);
static const uint64_t SCE_VIDEO_OUT_BUFFER_ATTRIBUTE_OPTION_MASK =
    (SCE_VIDEO_OUT_BUFFER_ATTRIBUTE_OPTION_STRICT_COLORIMETRY |
     SCE_VIDEO_OUT_BUFFER_ATTRIBUTE_OPTION_ALPHA_PREMULTIPLIED);
...
typedef struct SceVideoOutFlipStatus {
  ...
  uint64_t reserved1;
  uint64_t processTimeCounter; // processTimeCounter value of the time the
                               // latest flip executed
  int32_t gcQueueNum; // number of non-finished flips that are waiting for the
                      // setFlip command to execute on the GPU
  int32_t flipPendingNum; // number of total non-finished flips
  int32_t currentBuffer;  // current buffer index on display
  uint32_t reserved2;
  uint64_t
      submitProcessTimeCounter; // Process Time Counter Value value of the time
                                // the latest flip was requested on the GPU
...
typedef struct SceVideoOutVblankStatus {
  ...
  uint64_t processTimeCounter; // ProcessTimeCounter value when the latest
                               // vblank event
...
void sceVideoOutSetBufferAttribute2(
    struct SceVideoOutBufferAttribute2 *attribute, uint64_t pixelFormat,
    uint32_t tilingMode, uint32_t width, uint32_t height, uint64_t option,
    uint32_t dccControl, uint64_t dccCbRegisterClearColor);

int32_t sceVideoOutRegisterBuffers2(
    int32_t handle, int32_t setIndex, int32_t bufferIndexStart,
    const struct SceVideoOutBuffers *buffers, int32_t bufferNum,
    const struct SceVideoOutBufferAttribute2 *attribute, int32_t category,
    void *option);
...
static const uint64_t SCE_VIDEO_OUT_PIXEL_FORMAT2_R8_G8_B8_A8_SRGB =
    0x8000000022000000UL;
static const uint64_t SCE_VIDEO_OUT_PIXEL_FORMAT2_B8_G8_R8_A8_SRGB =
    0x8000000000000000UL;
static const uint64_t SCE_VIDEO_OUT_PIXEL_FORMAT2_R10_G10_B10_A2 =
    0x8100000622000000UL;
static const uint64_t SCE_VIDEO_OUT_PIXEL_FORMAT2_B10_G10_R10_A2 =
    0x8100000600000000UL;
static const uint64_t SCE_VIDEO_OUT_PIXEL_FORMAT2_R10_G10_B10_A2_SRGB =
    0x8100000022000000UL;
static const uint64_t SCE_VIDEO_OUT_PIXEL_FORMAT2_B10_G10_R10_A2_SRGB =
    0x8100000000000000UL;
static const uint64_t SCE_VIDEO_OUT_PIXEL_FORMAT2_R10_G10_B10_A2_BT2100_PQ =
    0x8100070422000000UL;
static const uint64_t SCE_VIDEO_OUT_PIXEL_FORMAT2_B10_G10_R10_A2_BT2100_PQ =
    0x8100070400000000UL;
static const uint64_t SCE_VIDEO_OUT_PIXEL_FORMAT2_R16_G16_B16_A16_FLOAT =
    0xC001000622000000UL;
static const uint64_t SCE_VIDEO_OUT_PIXEL_FORMAT2_B16_G16_R16_A16_FLOAT =
    0xC001000600000000UL;
static const uint64_t
    SCE_VIDEO_OUT_PIXEL_FORMAT2_R16_G16_B16_A16_FLOAT_FOR_BT2100_PQ =
        0xC001070722000000UL;
static const uint64_t
    SCE_VIDEO_OUT_PIXEL_FORMAT2_B16_G16_R16_A16_FLOAT_FOR_BT2100_PQ =
        0xC001070700000000UL;
...
int32_t sceVideoOutGetVblankStatus(int32_t handle,
...
int32_t sceVideoOutAddFlipEvent(SceKernelEqueue eq, int32_t handle,
                                void *udata);
int32_t sceVideoOutAddVblankEvent(SceKernelEqueue eq, int32_t handle,
                                  void *udata);
...
int32_t sceVideoOutAddPreVblankStartEvent(SceKernelEqueue eq, int32_t handle,
                                          void *udata);
int32_t sceVideoOutDeletePreVblankStartEvent(SceKernelEqueue eq,
                                             int32_t handle);
...
int sceKernelAddWriteEvent(SceKernelEqueue eq, int fd, size_t size,
                           void *udata);
...
typedef struct ScePadData {
  ...
  SceFQuaternion
      orientation;          /*E Controller orientation as a Quaternion <x,y,z,w>
                             *E   It is calculated as an accumulation value when
                             *controller is connected.          E   It can reset by
                             *scePadResetOrientation function. */
  SceFVector3 acceleration; /*E Acceleration of the controller(G).    */
  SceFVector3
      angularVelocity; /*E Angular velocity of the controller(radians/s). */
  ...
  ScePadExtensionUnitData
      extensionUnitData; /*E Data area for retrieve the extension unit. */
  uint8_t connectedCount;
  uint8_t reserve[2];          /*E Reserved area */
  uint8_t deviceUniqueDataLen; /*E Device unique data length(for special
                                  controllers) */
  uint8_t
      deviceUniqueData[SCE_PAD_MAX_DEVICE_UNIQUE_DATA_SIZE]; /*E Device unique
                                                                data(for special
                                                                controllers) */
...
extern "C" {
int scePadOpen(int userId, int32_t type, int32_t index,
