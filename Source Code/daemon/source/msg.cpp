...
void LoadSettings() {
  ...
  const char* config_path = "/data/PIZZA_HEN/config.ini";
  ...
  if (stat(config_path, &file_stat) != 0) {
    ...
    if (fd >= 0) {
      ...
      close(fd);
      etaHEN_log("PIZZA HEN config created @ /data/PIZZA_HEN/config.ini");
...
bool HookGame(UniquePtr<Hijacker>& hijacker, uint64_t alsr_b) {
    ...
    strcpy(stuff.prx_path, "/data/PIZZA_HEN/fps.prx");
...
static const char *PIZZAHEN_TOOLBOX_RESIDENT = "/system_tmp/pizzahen_toolbox_resident";

static int read_toolbox_resident_pid(void) {
    FILE *fp = fopen(PIZZAHEN_TOOLBOX_RESIDENT, "r");
    if (!fp) return -1;
    int pid = -1;
...
static void write_toolbox_resident_pid(int pid) {
    FILE *fp = fopen(PIZZAHEN_TOOLBOX_RESIDENT, "w");
    if (!fp) return;
    fprintf(fp, "%d\n", pid);
    fflush(fp);
    fsync(fileno(fp));
...
bool cmd_enable_toolbox(){
    ...
    // PID so a stale marker from a ShellUI restart is automatically ignored.
    int current_shellui_pid = get_shellui_pid();
    ...
    if (resident_pid > 0 && resident_pid != current_shellui_pid) {
      etaHEN_log("PIZZA HEN Toolbox resident marker stale (%d != %d); clearing", resident_pid, current_shellui_pid);
      unlink(PIZZAHEN_TOOLBOX_RESIDENT);
    }

    // Historical preloaded marker: if it ever exists, use the same safe
    // native reopen path instead of attempting another injection.
    ...
    if (!Inject_Toolbox(pid, shellui_elf_start)) {
      pause_resume_kstuff();
      etaHEN_log("PIZZA HEN Toolbox injection failed; ShellUI left running");
      notify(true, "Failed to inject the PIZZA HEN Toolbox");
      return false;
    }
    ...
    while (!if_exists("/system_tmp/toolbox_online")) {
      ...
      sleep(1);
      if(++wait >= 15){
        etaHEN_log("PIZZA HEN Toolbox start timeout; ShellUI left running");
        pause_resume_kstuff();
        notify(true, "Failed to load the PIZZA HEN Toolbox");
        return false;
      }
    }
    // The injected Toolbox is now resident in this ShellUI process. Persist
    // only the PID association, then consume the one-shot startup handshake.
    write_toolbox_resident_pid(pid);
    ...
    // a later tile click safely reuses the resident payload and retries the URI.
    return true;
}
