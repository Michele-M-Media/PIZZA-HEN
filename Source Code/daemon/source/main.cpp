...
void patch_checker();
void *app_plugin_monitor_thread(void *args);
...
void etaHEN_log(const char *fmt, ...) {
    ...
    int fd = open("/data/PIZZA_HEN/etaHEN.log", O_WRONLY | O_CREAT | O_APPEND, 0777);
    if (fd < 0) {
        return;
    }
...
void sig_handler(int signo) {
    ...
    notify(true,
          "etaHEN has crashed ...\n\nPlease send /data/PIZZA_HEN/etaHEN_crash.log "
...
int main() {
    ...
    pthread_t app_plugin_thr = nullptr;
    ...
    unlink("/data/PIZZA_HEN/etaHEN.log");
    unlink("/data/PIZZA_HEN/etaHEN_crash.log");
    ...
    // etaHEN 2.6B delta: application-specific plugin autoload monitor.
    // Runs as an additive PIZZA HEN service; it does not replace the existing daemon.
    // Lite Mode intentionally keeps application plugin injection disabled.
    if (!is_lite) {
        pthread_create(&app_plugin_thr, nullptr, app_plugin_monitor_thread, nullptr);
        pthread_detach(app_plugin_thr);
    }

    etaHEN_log("is toolbox only: %s | ver: %x", toolbox_only ? "Yes" : "No", sys_ver.version);
    // FIX32: PIZZA HEN uses the permanent Media tile as the primary Toolbox entry.
    // Keep etaHEN's native Toolbox available on demand, but never inject it during boot.
    const bool browser_toolbox_mode = if_exists("/data/PIZZA_HEN/runtime/browser_toolbox_mode");
    if (browser_toolbox_mode) {
    ...
    else if (!browser_toolbox_mode) {
        notify(true, "the PIZZA HEN Toolbox auto start is disabled in the config.ini\n\n"
    ...
    if (global_conf.PS5Debug && !no_ps5debug && !has_hv_bypass && !is_lite) {
     ...
     "      \"actions\": [\n"
     "        {\n"
     "          \"actionName\": \"Open PIZZA HEN Toolbox\",\n"
     "          \"actionType\": \"DeepLink\",\n"
     "          \"defaultFocus\": true,\n"
     "          \"parameters\": {\n"
