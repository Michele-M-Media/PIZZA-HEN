...
void notify(bool show_watermark, const char *text, ...) {
  ...
  if (show_watermark)
    snprintf(req.message, sizeof(req.message), "[PIZZA HEN] %s", buff);
  else
...
void *Play_time_thread(void *args) noexcept {
  const char *filename = "/data/PIZZA_HEN/playtime.bin";
  std::string tid;
  uint64_t duration = 0;
  int appid;
  ...
  while (true) {
      ...
      if (find_pid("etaHEN Utility") < 0 && retries < MAX_RETIRES) {
          if (retries == 0 || !util_elf) {
              ...
              if (!Open_Utility_Elf("/data/PIZZA_HEN/daemons/util.elf", &util_elf)) {
