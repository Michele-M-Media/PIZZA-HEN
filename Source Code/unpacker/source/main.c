...
static void ui_trace(const char *text, ...) {
  char line[1024] = {0};
  va_list args;
  va_start(args, text);
...
int main() {
  ...
  ui_trace("PIZZA HEN DIAG U1: unpacker entered");

  if (etahen_compressed_size <= 0) {
    printf("Invalid PIZZA HEN payload! unable to unpack it!");
    return 0;
  }
  ...
  if (!decompressed) {
    notify("Failed to allocate memory for decompressed PIZZA HEN payload!");
    return -1;
  }
  ...
  if (res != 0) {
    notify("Failed to decompress PIZZA HEN payload! error: %d", res);
    free(decompressed);
    return -1;
  }

  ui_trace("PIZZA HEN DIAG U2: bootstrapper decompressed");

  puts("Bootstrapping PIZZA-HEN.elf...");

  mkdir("/data/PIZZA_HEN", 0777);
  // create the payload for exiting lite mode via Johns elf loader
  int fd = open("/data/PIZZA_HEN/PIZZA-HEN.bin", O_WRONLY | O_CREAT | O_TRUNC, 0666);
  if (fd >= 0) {
  ...
  if(!send_to_elfldr(decompressed, decompress_size)) {
    notify("The elfldr on port 9021 is REQUIRED for PIZZA HEN; make sure it is running and try again!");
    free(decompressed);
    return -1;
  }

  ui_trace("PIZZA HEN DIAG U3: bootstrapper sent to elfldr");
