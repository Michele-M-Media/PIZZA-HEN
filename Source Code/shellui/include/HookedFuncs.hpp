...
// etaHEN 2.6B delta: application-specific plugin entry.  The persistent state
// is stored in plugins.ini by adding/removing the ?autoload suffix.
typedef struct {
    std::string section;
    std::string path;
    std::string id;
...
enum Cheats_Shortcut{
 ...
 enum cheats_repo_source{
    CHEATS_REPO_HEN_COLLECTION = 0,
    CHEATS_REPO_ETAHEN,
    CHEATS_REPO_GOLDHEN,
    CHEATS_REPO_RDX_PPSA
 };
...
typedef struct etaHENSettings_t
{
    ...
    bool overlay_gpu = false;
    bool overlay_cpu = false;
    ...
    cheats_repo_source selected_cheats_repo = CHEATS_REPO_HEN_COLLECTION;
...
bool set_app_plugin_autoload(const AppPluginEntry& entry, bool enabled);
...
extern  std::vector<AppPluginEntry> app_plugins_list;
