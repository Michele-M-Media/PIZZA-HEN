...
std::vector<AppPluginEntry> app_plugins_list;
std::vector<Payloads_Apps> payloads_apps_list, custom_pkg_list;
Payloads_Apps custom_pkg_path = { .path = "/data/PIZZA_HEN/pkgs" };
...
static const char *INI_PATH = "/user/data/PIZZA_HEN/config.ini";
...
bool LoadSettings()
{
  ...
  if (ini_parser_load(&parser, INI_PATH))
  {
    ...
    const char *FTP_str = ini_parser_get(&parser, "Settings.FTP", "0");
    ...
    const char *auto_eject_disc_str = ini_parser_get(&parser, "Settings.auto_eject_disc", "0");
	  const char* overlay_ram = ini_parser_get(&parser, "Settings.overlay_ram", "0");
	  const char* overlay_cpu = ini_parser_get(&parser, "Settings.overlay_cpu", "0");
	  const char* overlay_gpu = ini_parser_get(&parser, "Settings.overlay_gpu", "0");
	...
	global_conf.overlay_ram = overlay_ram ? atoi(overlay_ram) : 0;
	global_conf.overlay_cpu = overlay_cpu ? atoi(overlay_cpu) : 0;
	global_conf.overlay_gpu = overlay_gpu ? atoi(overlay_gpu) : 0;
  ...
  if (global_conf.overlay_fps ){
    ...
    // FIX43: CPU/GPU/RAM overlay sections are opt-in. Earlier pre-release
    // builds defaulted all three to enabled. Migrate that exact legacy
    // all-enabled state once, while preserving already-customized users.
    const char *overlay_defaults_marker = "/user/data/PIZZA_HEN/.overlay_defaults_opt_in_v1";
    if (!if_exists(overlay_defaults_marker)) {
      if (global_conf.overlay_ram && global_conf.overlay_cpu && global_conf.overlay_gpu) {
        global_conf.overlay_ram = false;
        global_conf.overlay_cpu = false;
        global_conf.overlay_gpu = false;
        SaveSettings();
        shellui_log("PIZZA HEN: migrated legacy overlay defaults to opt-in");
      }
      touch_file(overlay_defaults_marker);
    }
...
void generate_remote_play_xml(std::string &xml_buffer)
{
  ...
  if (IsNotActivated())
  {
    ...
    xml_buffer += R"(<label id="id_pin_2" title="Account activated by PIZZA HEN, please reboot your console before using Remote Play!" style="center"/>)";
    goto close;
  }
...
void generate_plugin_xml(std::string &xml_buffer, bool plugins_xml)
{
  ...
  std::vector<std::string> directories = {
      "/user/data/PIZZA_HEN/plugins",
      "/usb0/PIZZA_HEN/plugins",
      "/usb1/etaHEN/plugins",
      "/usb2/etaHEN/plugins",
      "/usb3/etaHEN/plugins",

      "/user/data/PIZZA_HEN/payloads",
      "/usb0/PIZZA_HEN/payloads",
  ...
  if (plugins_xml)
  {
    xml_buffer += "<link id=\"id_auto_plugins\" title=\"★ Plugins - Startup Menu\" file=\"auto_plugins.xml\" second_title=\"Configure plugins to launch when you load PIZZA HEN\"/>\n";
    xml_buffer += "<link id=\"id_app_plugins\" title=\"★ PIZZA HEN App Plugin Manager *BETA*\" file=\"app_plugins.xml\" second_title=\"Configure saved autoload rules and inspect live plugin runtime state\"/>\n";
    xml_buffer += "</setting_list>\n</setting_list>\n</system_settings> ";
  }
  else
  {
...
static std::string app_plugin_trim(const std::string& in)
{
  size_t first = 0;
...
static bool app_plugin_ends_with(const std::string& value, const std::string& suffix)
{
  return value.size() >= suffix.size() &&
         value.compare(value.size() - suffix.size(), suffix.size(), suffix) == 0;
}

static std::string app_plugin_xml_escape(std::string value)
{
  ...
  for (const auto& pair : map) {
    ...
    while ((pos = value.find(pair.first, pos)) != std::string::npos) {
      value.replace(pos, strlen(pair.first), pair.second);
...
static const char* resolve_app_plugins_config_path()
{
  static const char* primary = "/data/PIZZA_HEN/plugins/apps/plugins.ini";
  static const char* shell_view = "/user/data/PIZZA_HEN/plugins/apps/plugins.ini";
  if (access(primary, F_OK) == 0)
    return primary;
  if (access(shell_view, F_OK) == 0)
    return shell_view;
  return primary;
}

static const char* resolve_app_plugins_status_path()
{
  static const char* primary = "/data/PIZZA_HEN/runtime/app_plugins_status.txt";
  static const char* shell_view = "/user/data/PIZZA_HEN/runtime/app_plugins_status.txt";
  if (access(primary, F_OK) == 0)
    return primary;
  if (access(shell_view, F_OK) == 0)
    return shell_view;
...
static AppPluginRuntimeStatus read_app_plugin_runtime_status()
{
  AppPluginRuntimeStatus status;
  std::ifstream in(resolve_app_plugins_status_path());
  if (!in.is_open())
    return status;
  ...
  while (std::getline(in, line)) {
    ...
    else if (key == "title_id") status.title_id = value;
    else if (key == "pid") status.pid = value;
    else if (key == "configured") status.configured = value;
    else if (key == "armed") status.armed = value;
    else if (key == "loaded") status.loaded = value;
    else if (key == "current") status.current = value;
    else if (key == "last_result") status.last_result = value;
  }
  return status;
}

static bool read_app_plugins_ini(std::vector<AppPluginEntry>& entries)
{
  entries.clear();
  const char* config_path = resolve_app_plugins_config_path();
  ...
  while (std::getline(in, line)) {
    if (!line.empty() && line.back() == '\r')
      line.pop_back();
    std::string trimmed = app_plugin_trim(line);
    if (trimmed.empty() || trimmed[0] == ';' || trimmed[0] == '#')
      continue;

    if (trimmed.size() >= 3 && trimmed.front() == '[' && trimmed.back() == ']') {
      section = app_plugin_trim(trimmed.substr(1, trimmed.size() - 2));
      if (section.empty())
        section = "DEFAULT";
      continue;
    }
    ...
    entry.autoload = app_plugin_ends_with(trimmed, "?autoload");
    entry.path = entry.autoload ? trimmed.substr(0, trimmed.size() - 9) : trimmed;
    entry.path = app_plugin_trim(entry.path);
    entry.id = "id_app_plugin_" + std::to_string(index++);
    if (!entry.path.empty())
...
void generate_app_plugins_xml(std::string& xml_buffer)
{
  app_plugins_list.clear();
  const bool config_ok = read_app_plugins_ini(app_plugins_list);
  const AppPluginRuntimeStatus runtime = read_app_plugin_runtime_status();

  std::string running_app;
  int appid = -1;
  const bool app_open = Get_Running_App_TID(running_app, appid);

  xml_buffer = "<?xml version=\"1.0\" encoding=\"UTF-8\" ?>\n"
               "<system_settings version=\"1.0\" plugin=\"debug_settings_plugin\">\n"
               "<setting_list id=\"id_app_plugins\" title=\"★ PIZZA HEN App Plugin Manager *BETA*\">\n";

  xml_buffer += "<label id=\"id_app_plugins_info\" title=\"Application plugin manager: saved autoload state and live runtime state are tracked separately\" style=\"center\"/>\n";
  xml_buffer += "<label id=\"id_app_plugins_path\" title=\"Config: /data/PIZZA_HEN/plugins/apps/plugins.ini\" style=\"center\"/>\n";
  if (app_open)
    xml_buffer += "<label id=\"id_app_plugins_running\" title=\"Running application: " + app_plugin_xml_escape(running_app) + "\" style=\"center\"/>\n";
  else
    xml_buffer += "<label id=\"id_app_plugins_running\" title=\"No application is currently running\" style=\"center\"/>\n";

  xml_buffer += "<label id=\"id_app_plugins_runtime\" title=\"Runtime: " +
                app_plugin_xml_escape(runtime.state + " | TID " + runtime.title_id + " | PID " + runtime.pid) +
                "\" style=\"center\"/>\n";
  xml_buffer += "<label id=\"id_app_plugins_counts\" title=\"Matching: " +
                app_plugin_xml_escape(runtime.configured + " configured | " + runtime.armed + " armed | " + runtime.loaded + " loaded") +
                "\" style=\"center\"/>\n";
  if (runtime.last_result != "-" || runtime.current != "-") {
    xml_buffer += "<label id=\"id_app_plugins_last\" title=\"Last: " +
                  app_plugin_xml_escape(runtime.last_result + " | " + runtime.current) +
                  "\" style=\"center\"/>\n";
  }

  if (!config_ok) {
    xml_buffer += "<label id=\"id_app_plugins_missing\" title=\"plugins.ini is not ready yet. Reopen this menu after PIZZA HEN finishes starting.\" style=\"center\"/>\n";
  } else if (app_plugins_list.empty()) {
    xml_buffer += "<label id=\"id_app_plugins_empty\" title=\"No App Plugins configured. Add paths under [DEFAULT] or a game Title ID.\" style=\"center\"/>\n";
  } else {
    for (const auto& entry : app_plugins_list) {
      const std::string title = app_plugin_xml_escape(entry.section + " — " + entry.path);
      std::string second = entry.autoload ? "ARMED for autoload" : "DISARMED";
      second += " in [" + entry.section + "]";
      if (entry.section == "DEFAULT")
        second += " (all applications)";
      second += ". Disabling changes future loads only; a loaded plugin remains until the app closes.";
      xml_buffer += "<toggle_switch id=\"" + entry.id + "\" title=\"" + title +
                    "\" second_title=\"" + app_plugin_xml_escape(second) + "\" value=\"0\"/>\n";
    }
  }

  xml_buffer += "<label id=\"id_app_plugins_hint\" title=\"Append ?autoload to arm a plugin. Runtime state refreshes automatically while the application is active.\" style=\"center\"/>\n";
  xml_buffer += "</setting_list>\n</system_settings>";
}

bool set_app_plugin_autoload(const AppPluginEntry& target, bool enabled)
{
  const char* config_path = resolve_app_plugins_config_path();
  std::ifstream in(config_path);
  if (!in.is_open())
  ...
  bool changed = false;
  for (auto& raw : lines) {
    std::string trimmed = app_plugin_trim(raw);
    if (trimmed.empty() || trimmed[0] == ';' || trimmed[0] == '#')
      continue;
    if (trimmed.size() >= 3 && trimmed.front() == '[' && trimmed.back() == ']') {
      section = app_plugin_trim(trimmed.substr(1, trimmed.size() - 2));
    ...
    const bool has_autoload = app_plugin_ends_with(trimmed, "?autoload");
    std::string path = has_autoload ? trimmed.substr(0, trimmed.size() - 9) : trimmed;
    path = app_plugin_trim(path);
    if (section == target.section && path == target.path) {
      raw = target.path + (enabled ? "?autoload" : "");
      changed = true;
      break;
    }
  }
  ...
  const std::string temp_path = std::string(config_path) + ".tmp";
...
void generate_cheats_xml(std::string &new_xml, std::string& not_open_tid, bool running_as_debug_settings, bool show_while_not_open)
{
  ...
  // Use a custom raw-string delimiter because the UI label contains the sequence )".
  std::string dl_cheats = R"PIZZA(<list id="id_selected_cheats_repo" title="PIZZA HEN Cheat Repository" >
                   <list_item id="id_selected_cheats_repo_1" title="Unified Collection (HEN-Cheats-Collection) - Recommended" value="0"/>
                   <list_item id="id_selected_cheats_repo_2" title="PS5_Cheats Upstream Repository" value="1"/>
                   <list_item id="id_selected_cheats_repo_3" title="GoldHEN Cheat Repository" value="2"/>
                   <list_item id="id_selected_cheats_repo_4" title="RDX HEN-PPSA-Cheats (PS5 native only)" value="3"/>
                 </list>
                <button id="id_dl_cheats" title="Download/Update Selected Repository" second_title="Updates the selected upstream cheat collection and refreshes the PIZZA HEN cheat cache"/>)PIZZA";

  std::string reload_cheats = R"(<button id="id_reload_cheats" title="Cache and reload Cheats list" second_title="New cheats added to /data/PIZZA_HEN/cheats/EXT_HERE will be cached and the cheats list will be reloaded"/>)";
  //
  ...
  {
    new_xml += "<setting_list id=\"" + list_id + "\" title=\"PIZZA HEN Cheats - No "
               "Game is open\">\n";
    new_xml += dl_cheats;
  ...
  else
  {
    ...
    new_xml += "<setting_list id=\"" + list_id + "\" title=\"PIZZA HEN Cheats - ";
...
void generate_plapps_xml(std::string& new_xml) {
  ...
  std::vector<std::string> directories = {
    ...
    new_xml += "<setting_list id=\"id_plapps\" title=\"PIZZA HEN Payload Homebrew - Applications\">\n";
