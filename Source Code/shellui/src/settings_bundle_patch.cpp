...
static constexpr unsigned char kLegacyNewLabel[] = {
    0xe2, 0x98, 0x85, 'P', 'I', 'Z', 'Z', 'A', ' ', 'H', 'E', 'N', ' ', 'M',
    'e', 'n', 'u'};
...
static constexpr unsigned char kHermesNewLabel[] = {
    0x05, 0x26, 'P', 0x00, 'I', 0x00, 'Z', 0x00, 'Z', 0x00,
    'A',  0x00, ' ', 0x00, 'H', 0x00, 'E', 0x00, 'N', 0x00,
    ' ',  0x00, 'M', 0x00, 'e', 0x00, 'n', 0x00, 'u', 0x00,
};
...
void patch_settings_bundle(unsigned char *buffer, int size) {
  ...
  if (locate_legacy_payload(buffer, static_cast<size_t>(size), &legacy)) {
    ...
    const int label_count = apply_equal_length_patch(
        ...
        sizeof(kLegacyOldLabel));
#if SHELL_DEBUG == 1
    shellui_log("settings_bundle_patch: activated '%s' label=%d "
...
#if SHELL_DEBUG == 1
  shellui_log("settings_bundle_patch: Hermes NPXS40008 label=%d footer=%s "
