...
static const char* CHEAT_REPO_COMMIT_URLS[] = {
    "https://api.github.com/repos/TeeKay87/HEN-Cheats-Collection/commits/master",
    "https://api.github.com/repos/etaHEN/PS5_Cheats/commits/main",
    "https://api.github.com/repos/GoldHEN/GoldHEN_Cheat_Repository/commits/main",
    "https://api.github.com/repos/RDX-Sci01/HEN-PPSA-Cheats/commits/main"
};
static const char* CHEAT_REPO_HASH_FILES[] = {
...
static bool extract_zip_internal(const char* zip_path, const char* extract_dir, const char* subroot) {
    ...
    do {
        ...
        // Optional source-root filter. Used by repositories that keep the
        // etaHEN-ready database under a top-level cheats/ directory.
        if (subroot && subroot[0]) {
            size_t subroot_len = strlen(subroot);
            if (strncmp(actual_filename, subroot, subroot_len) != 0) {
                processed_files++;
                continue;
            }
            actual_filename += subroot_len;
...
bool extract_zip(const char* zip_path, const char* extract_dir) {
    return extract_zip_internal(zip_path, extract_dir, NULL);
}

bool extract_zip_subdir(const char* zip_path, const char* extract_dir, const char* subroot) {
    return extract_zip_internal(zip_path, extract_dir, subroot);
}
...
static bool read_stored_commit_hash(const char* hash_path, char* hash_buffer, size_t buffer_size) {
    int fd = sceKernelOpen(hash_path, O_RDONLY, 0);
    if (fd < 0) {
...
static bool write_commit_hash(const char* hash_path, const char* hash) {
    int fd = sceKernelOpen(hash_path, O_WRONLY | O_CREAT | O_TRUNC, 0644);
    if (fd < 0) {
...
bool check_for_new_commit(int repo) {
    ...
    if (repo < 0 || repo >= CHEAT_REPO_COUNT) {
        etaHEN_log("Invalid cheat repository id %d", repo);
        return false;
    }

    // Download the latest commit information
    json_data = download_json(CHEAT_REPO_COMMIT_URLS[repo]);
    if (!json_data) {
    ...
    bool has_stored_hash = read_stored_commit_hash(CHEAT_REPO_HASH_FILES[repo], stored_commit, sizeof(stored_commit));
    if (!has_stored_hash) {
    ...
    if (has_new_commit) {
        if (!write_commit_hash(CHEAT_REPO_HASH_FILES[repo], latest_commit)) {
