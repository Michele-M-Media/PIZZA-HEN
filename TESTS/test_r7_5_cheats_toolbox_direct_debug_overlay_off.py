...
ck('R74_SHELLUI_HOOKS_R75_INTENTIONAL_DELTA', '★ PIZZA HEN Cheats' in hook.read_text(errors='ignore') and 'toolbox-launcher.html?tid=' in hook.read_text(errors='ignore'))
...
# R7.5: ★ PIZZA HEN Cheats is owned by the modern Toolbox, not Debug Services.
hook_text=hook.read_text(errors='ignore')
prx=(SRC/'shellui/src/prx.cpp').read_text(errors='ignore')
ck('R75_CHEATS_DIRECT_TOOLBOX_URL', 'http://127.0.0.1:8080/fs/data/PIZZA_HEN/ui/toolbox-launcher.html?tid=' in hook_text and 'cheats_uri += "#cheats"' in hook_text)
block=hook_text[hook_text.index('if(id_str == "MENU_ID_CHECK_PATCH")'):hook_text.index('if(id_str == "MENU_ID_INTELLECTUAL_PROPERTY_NOTICES")')]
ck('R75_GAME_OPTIONS_NO_LEGACY_CHEATS_URI', 'etaHEN?Cheats_not_open' not in block)
ck('R75_CHEATS_CONTEXT_TID', 'current_menu_tid' in block)
ck('R75_CHEATS_CONTEXT_PID', 'find_pid(current_menu_tid.c_str(), false, true, true)' in block)
ck('R75_API_EXISTING_CHEAT_ABI', '#define BREW_UTIL_GET_GAME_VER    0x08000009' in api and '#define BREW_UTIL_GET_GAME_CHEAT  0x0800000A' in api and '#define BREW_UTIL_TOGGLE_CHEAT    0x0800000B' in api)
ck('R75_API_CHEATS_LOAD', 'if(!strcmp(action,"cheats-load"))' in api and 'send_ipc_reply(UTIL_IPC_SOC,BREW_UTIL_GET_GAME_VER' in api and 'send_ipc_reply(UTIL_IPC_SOC,BREW_UTIL_GET_GAME_CHEAT' in api)
ck('R75_API_CHEAT_TOGGLE', 'if(!strcmp(action,"cheat-toggle"))' in api and 'send_ipc(UTIL_IPC_SOC,BREW_UTIL_TOGGLE_CHEAT' in api)
ck('R75_WEB_CHEAT_SESSION', 'cheat_session.json' in html and 'async function loadGameCheats()' in html and 'async function updateCheatRepository()' in html)
ck('R75_WEB_CHEAT_TOGGLES', "runAction('cheat-toggle '+tid+' '+pid+' '+Number(ch.id))" in html)
ck('R75_DEBUG_OVERLAY_FORCE_OFF_SCOPE', 'if (!game_options_service_requested)' in prx and 'Debug Services legacy overlays forced OFF' in prx)
for field in ['overlay_ram','overlay_cpu','overlay_gpu','overlay_fps','overlay_ip','overlay_kstuff','overlay_kstuff_active','all_cpu_usage']:
    ck('R75_DEBUG_OVERLAY_OFF_'+field.upper(), f'global_conf.{field} = false;' in prx)
ck('R75_DEBUG_FPS_MARKER_OFF', 'unlink("/system_tmp/fps_enabled")' in prx)
ck('R75_PHU_OVERLAY_UNTOUCHED_BY_DEBUG', 'unlink("/data/phu_overlay.lock")' not in prx and '/system_tmp/fps_enabled' in prx)
ck('R75_DEBUG_BRIDGE_STILL_FROZEN', sha(SRC/'bootstrapper/assets/debug_services_launcher.html')=='7f7134593eefa9628bc581eebe3a7fc66f40cba3bb8f9447ebd641bfe58eb399' and sha(SRC/'toolbox_action/src/main.c')=='8155569ab893e23d365b054d8c3075fcdebb6792b75f0ccf21d2bff33f76faf6')
ck('R75_KSTUFF_BOOT_LINEAGE_FROZEN', sha(SRC/'bootstrapper/source/main.cpp')=='cf202070d73c50bcd5529c8895a93f9ef1132d07c397c51a5a560e91cd2b8419' and sha(ROOT/'KSTUFF_INPUT/kstuff-v1.10-normal.elf')=='b1dfe57f367a35374f605127915eda38c76a6ed5d1c729e427955798bd78c66a')
print('R7_5_CHEATS_TOOLBOX_DIRECT_DEBUG_OVERLAY_OFF=PASS')
